...
public:
	...
	//! Templateに基づいたファイルの生成をするかを返す
	/*!
	 * \return 要否（true：要、false：否）
	 */
	const bool isUsingTemplate() const;
protected:
	...
	bool usingTemplate_; //!< テンプレート使用の要否
...
private:
	...
	//! テンプレートを基にファイルを生成するかどうか
	/*!
	 * \param argument 判定対象のオプション
	 * \return 判定結果（true：テンプレートを使用する、false：使用しない）
	 */
	bool isTemplateUse(std::string argument);
